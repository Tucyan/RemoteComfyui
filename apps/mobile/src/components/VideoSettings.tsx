...
import { adjustVideoFrames, calculateVideoDimensions, normalizeVideoDimensions, VIDEO_RATIOS } from "../domain/draft";
...
export function VideoSettings({ value, onChange }: Props) {
  ...
  const requestedWidth = value.custom ? Number(value.width) || calculated.width : calculated.width;
  const requestedHeight = value.custom ? Number(value.height) || calculated.height : calculated.height;
  const normalized = normalizeVideoDimensions(requestedWidth, requestedHeight);
  ...
  return <View style={styles.container}>
    ...
    <Text style={styles.sub}>目标总百万像素；当前约 {(calculated.width * calculated.height / 1_000_000).toFixed(2)} MP，{calculated.width} × {calculated.height}。</Text>
    ...
    <Text style={normalized.adjusted ? styles.warning : styles.sub}>实际生成尺寸：{normalized.width} × {normalized.height}{normalized.adjusted ? `（已从 ${requestedWidth} × ${requestedHeight} 自动调整为最近的 32 倍数）` : "（宽高是 32 的倍数）"}</Text>
    {(normalized.width > 1920 || normalized.height > 1088) && <Text style={styles.warning}>高分辨率不会被限制，但可能显著增加显存占用或导致生成失败。</Text>}
