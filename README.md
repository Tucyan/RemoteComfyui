...
这是一个 Windows 主机上的 ComfyUI 局域网网关。手机端生成页提供四个固定工作流：Qwen Edit 2511、Qwen Image 2.1 8GB Edit、Qwen Image 2.1 8GB 文生图和 MiniMax H3 视频；工作流 JSON、节点参数和本机绝对路径不会暴露给手机。
...
- **Qwen Edit 2511**：1 到 3 张参考图，保留原有图片编辑流程
- **Qwen Image 2.1 8GB Edit**：1 到 10 张参考图，第一张是编辑目标；可按原图 0.5×–2.0× 调整倍率，也可输入固定原图比例的宽高，实际尺寸对齐到 32 像素并限制最长边为 2048 像素
- **Qwen Image 2.1 8GB 文生图**：不使用参考图；可选七种比例、0.2–1.5 MP 目标清晰度，或输入自定义宽高，实际尺寸对齐到 8 像素并限制最长边为 2048 像素
- **MiniMax H3 视频**：1 到 9 张参考图，以及符合工作流步长的分辨率和帧数
- 生成页用四张工作流卡片切换；每个工作流分别保留自己的提示词和参考图。需要图片的工作流可从手机或电脑白名单图库选择；底部“图库”点图可查看原图并保存到手机相册
- 视频可选七种比例、目标清晰度 0.2–1.5 MP，或输入 32 对齐的自定义宽高；帧数可输入 5–362 中满足 17n+5 的值，也可用按钮微调
- 产物可在手机内预览、保存和分享：图片支持双指缩放及放大后单指拖动，视频支持播放；点击删除并确认后，会永久删除 Gateway 数据目录中的该产物文件及记录，不删除电脑图库原图或 ComfyUI 原始输出
